...
| `DENIED` | Permanently blocked by policy or approval rejection path | Emitted for policy `DENY` and current rejected-approval retry path | Stop and reformulate, narrow scope, or abandon | Do not retry the same governed action unless the policy context or request changes materially | Current deny path records `POLICY_CHECKED` and `OUTCOME_GENERATED`; rejected approval retries also record `APPROVAL_DENIED` |
